...
- **aiohttp** — HTTP-клиент для Beszel REST API + SSE
...
| `SSE_RECONNECT_SECONDS` | Пауза перед реконнектом при обрыве SSE | `5` |
...
### Мониторинг
- Realtime через **PocketBase SSE** (`/api/realtime`) — события приходят мгновенно
- Подписка на коллекцию `systems/*`: create / update / delete
- При изменении статуса ноды — немедленный алерт
- При обрыве SSE — реконнект через `SSE_RECONNECT_SECONDS`, REST-синхронизация пропущенных изменений
